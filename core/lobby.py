...
def _do_roblox_rejoin(reason: str = "Unknown") -> bool:
    ...
    while time.time() < deadline:
        ...
        if detections._wait_for_image("IsInGame.png", timeout=3.0, confidence=0.8):
            ...
            helpers.align_roblox(padding=10)
...
def prepare_lobby() -> bool:
    ...
    InputHandler.Click(653 + state.dx, 189 + state.dy, delay=0.1)
...
def lobby_path_cid_raid() -> bool:
    ...
    start_btn = detections._wait_for_image("StartButton.png", timeout=15.0, confidence=0.7)
    if start_btn:
